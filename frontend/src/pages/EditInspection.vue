<template>
  <div class="page">
    ...
    <nav class="breadcrumb">
      ...
      <span class="sep">›</span>
      <span class="active">Edit Inspection</span>
    </nav>
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <span class="inspection-no">{{ inspectionNo }}</span>
      </div>
...
import { fetchMasterData, fetchInspectionDetail, updateInspection } from '../services/inspection'
...
onMounted(async () => {
  try {
    const [mdData, inspection] = await Promise.all([
      fetchMasterData(),
      fetchInspectionDetail(route.params.id),
    ])

    masterData.value = {
      serviceTypes:  mdData.service_types  ?? [],
      scopes:        mdData.scopes         ?? [],
      scopeItems:    mdData.scope_items    ?? [],
      inventoryLots: mdData.inventory_lots ?? [],
    }

    if (inspection.workflow_status_group !== 'OPEN') {
      router.replace('/inspections/' + route.params.id)
      return
    }

    inspectionNo.value = inspection.inspection_no

    form.value = {
      service_type:              inspection.service_type || '',
      scope_of_work:             inspection.scope_of_work?.code || '',
      location:                  inspection.location || '',
      estimated_completion_date: inspection.estimated_completion_date || '',
      related_to:                inspection.related_to || '',
      charge_to_customer:        inspection.charge_to_customer ?? false,
      ...
      items: (inspection.items || []).map(item => ({
        ...
        lots: (item.lots || []).map(lot => ({
          ...
          sample_qty:    lot.sample_qty ?? 0,
        })),
      })),
    }
  } catch {
    error.value = 'Failed to load inspection data. Please try again.'
  } finally {
    loading.value = false
  }
})
...
async function submit() {
  ...
  saving.value = true
  try {
    await updateInspection(route.params.id, form.value)
...
.inspection-no { font-size: 14px; color: #64748b; font-weight: 500; }
